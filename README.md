...
There are two ways to get validator keys:

- **Option A (recommended):** Use Prysm''s deterministic interop keys -- fastest, no external tooling needed
- **Option B (advanced):** Generate wallet-based validator keys with the Ethereum Staking Deposit CLI Python source

> **Note:** The released Windows `deposit.exe` binary cannot generate valid deposits for this custom devnet because it does not support `--devnet_chain_setting` in `new-mnemonic`. If you want wallet-based keys, you must use the Python source method in Option B.
...
### Option B: Generate your own wallet-based validator keys with Python source

This is more realistic: you create a fresh mnemonic and derive real validator keystores. It requires Python 3.12+ and the staking-deposit-cli source.
...
Get-ChildItem -Path "wallet_setup\validator_keys" -Filter "keystore-m_12381_3600_*_0_0-*.json" | ForEach-Object {
    $idx = [int](# Private Ethereum Blockchain Setup on Windows using Geth 1.17 + Prysm (PoS)
...
#### Step 2 — Generate keys with the Windows binary
...
> **Why `--chain mainnet`?** The deposit CLI's `--chain` flag only selects the BLS signing domain. It does **not** connect you to real Ethereum. However, the Windows v2.8.0 binary does **not** support `--devnet_chain_setting` for `new-mnemonic`, so we use `--chain mainnet` and patch the fork version in the next step.
...
Patch the deposit data so it matches the local devnet's `GENESIS_FORK_VERSION` (`0x20000089`) from `chain-config.yaml`:
...
# Verify
Get-Content "wallet_setup\validator_keys\deposit_data.json" | ConvertFrom-Json | Select-Object -First 1 | Select-Object fork_version, network_name
```

Expected output:

```text
fork_version network_name
------------ ------------
20000089     localdev
```

> If the Windows binary crashes on the password prompt with a `UnicodeEncodeError`, use the Python source method in Step 2b instead.

#### Step 2b — Generate keys from Python source (if `deposit.exe` crashes)

The Python source supports `--devnet_chain_setting` directly, so no patching is needed.
...
.Name -split ''_'')[3]
    $dest = "wallet_setup\keys$($idx + 1)"
    Copy-Item -Path # Private Ethereum Blockchain Setup on Windows using Geth 1.17 + Prysm (PoS)
...
.FullName -Destination $dest -Force
    Write-Host "Copied $(# Private Ethereum Blockchain Setup on Windows using Geth 1.17 + Prysm (PoS)
